...
```mermaid
  graph TD;
    A["В поле 'Номер карты' ввести номер карты со статусом APPROVED 4444 4444 4444 4441"] --> B;
    style A fill:#3f3,stroke:#333,stroke-width:4px
    B["В поле 'Месяц' ввести любой месяц в формате двухзначного числа (например, 08)"] --> C;
    C["В поле 'Год' ввести любой год в формате двухзначного числа (например, 23)"] --> D;
    D["В поле 'Владелец' с помощью библиотеки faker ввести имя и фамилию латиницей (например, Olga Petrova) "] --> E;
    E["В поле 'CVC/CVV' ввести любое целое трехзначное число (например, 829)"] --> F;
